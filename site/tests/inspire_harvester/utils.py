...
def drop_legacy_cds_ids(payload):
    """Remove integer CDS recids so create tests use the no-legacy-recid path."""
    hits = (payload or {}).get("hits", {}).get("hits") or []
    for hit in hits:
        metadata = hit.get("metadata") or {}
        metadata["external_system_identifiers"] = [
            ident
            for ident in metadata.get("external_system_identifiers", [])
