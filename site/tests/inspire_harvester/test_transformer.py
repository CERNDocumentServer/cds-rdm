...
transformer_entry1 = {
    "metadata": {
        ...
        "accelerator_experiments": [
            ...
            {"accelerator": "Proton Synchrotron"},
            {"accelerator": "Linac 4"},
        ],
        "author_count": 1,
        "urls": [
...
def test_transformer(running_app):
    ...
    # ----- Custom fields -----
    # case 1: 2 cern:accelerators
    assert (
        transformer_entry1["metadata"]["accelerator_experiments"][2]["accelerator"]
        in record1["custom_fields"]["cern:accelerators"]
    )
    assert (
        transformer_entry1["metadata"]["accelerator_experiments"][3]["accelerator"]
        in record1["custom_fields"]["cern:accelerators"]
    )

    # case 2: 2 cern:experiments
    assert (
        transformer_entry1["metadata"]["accelerator_experiments"][0]["legacy_name"]
        in record1["custom_fields"]["cern:experiments"]
    )
    assert (
        transformer_entry1["metadata"]["accelerator_experiments"][1]["legacy_name"]
        in record1["custom_fields"]["cern:experiments"]
    )

    # case 3: imprint:imprint place
    assert {"place": transformer_entry4["metadata"]["imprints"][0]["place"]} == record4[
        "custom_fields"
    ]["imprint:imprint"]

    # case 4: imprint - ISBN with medium "online" and place
    assert {
    ...
    # case 5: imprint - 2 ISBNs with medium "online" (error)
    assert (
    ...
    # ----- Files -----
    # case 1: TODO: figures ignored
    # assert record1["files"]["enabled"] is False
    ...
    # case 4: TODO: no files present error
    # case 5: 2 files
    assert (
