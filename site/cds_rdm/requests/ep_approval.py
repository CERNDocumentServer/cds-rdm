...
def _set_committee_approval(draft_proxy, fields_dict):
    """Write committee approval fields directly to the draft record, bypassing schema.

    ``cern:committee_approval`` is system-managed and enforced as immutable by
    ``CommitteeApprovalComponent``.  This helper writes directly to the underlying
    record object so the value is present before the next service call triggers
    the component's restore logic.
    """
    rec = draft_proxy._record
    rec["custom_fields"] = rec.get("custom_fields") or {}
    existing = rec["custom_fields"].get(_CF_KEY) or {}
    rec["custom_fields"][_CF_KEY] = {**existing, **fields_dict}
    rec.commit()
...
def _family_recids(record) -> list:
    """Return all recid pid_values for every version in the record's family.

    Queries the DB directly so newly published versions not yet re-indexed
    are included.  Falls back to just the current record's recid on error.
    """
    try:
        parent = record.parent
        recids = []
        for family_rec in RDMRecord.get_records_by_parent(parent):
...
class EPApprovalSubmitAction(actions.CreateAndSubmitAction):
    ...
    def execute(self, identity: Identity, uow: UnitOfWork) -> None:
        ...
        # Reject if ANY version in the family already carries an approval report number.
        family_ids = _family_recids(topic)
        for recid in family_ids:
            pid = PersistentIdentifier.get("recid", recid)
            rec = RDMRecord.get_record(pid.object_uuid)
            if (rec.get("custom_fields", {}).get(_CF_KEY) or {}).get("reportnumber"):
                raise ValidationError(
        ...
        topic_query = " OR ".join(f'topic.record:"{r}"' for r in family_ids)
        existing = list(
...
class EPApprovalAcceptAction(actions.AcceptAction):
    ...
    def execute(self, identity: Identity, uow: UnitOfWork) -> None:
        """Execute accept: mint report number and write CF to the submitted record.

        Only the submitted (topic) record receives the ``committee_approval`` CF.
        Newer versions created from it will inherit the CF naturally via InvenioRDM's
        new_version metadata copy.  Pre-approval versions are left untouched.
        ...
        (detected by the presence of ``internally_reviewed_id`` in the CF).
        """
        ...
        # Write CF to the submitted record only.
        draft = records_service.edit(identity=system_identity, id_=submitted_version_recid)
        _set_committee_approval(draft, {
            "reportnumber": report_number,
            "datetime": datetime.now(timezone.utc).isoformat(),
            "version": submitted_version_recid,
        })
        records_service.publish(identity=system_identity, id_=draft.id)
